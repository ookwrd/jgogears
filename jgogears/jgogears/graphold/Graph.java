...
public final class Graph implements Cloneable {
	...
	public Node getNode(int row, int column) {
		if (row < 0 || column < 0 || row >= board.getSize()
	...
	public Graph(Graph g) {
		...
		while (i.hasNext()) {
			...
			while (j.hasNext()) {
				...
				newnode.addConnection(new Connection(first, second, c
						.getFirst_row(), c.getFirst_column(),
	...
	public Graph(BoardI board) {
		...
		for (short i = 0; i < board.getSize(); i++) {
			for (short j = 0; j < board.getSize(); j++) {
				if (i > 0) {
					Connection c = new Connection(vecvec.elementAt(i)
							.elementAt(j),
							vecvec.elementAt(i - 1).elementAt(j), i, j,
							(short) ((short) i - 1), j);
		...
		while (!done) {
			...
			while (!altered && i.hasNext()) {
				...
				while (!altered && j.hasNext()) {
					...
					/*
					 * if (first.compareTo(second) != 0 && first.getColour() ==
					 * second.getColour()) { merge(first, second); altered =
					 * true; }
					 */}
			}
			done = !altered;
