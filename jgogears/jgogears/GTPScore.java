...
public final class GTPScore {
    ...
    public String toString() {
        if (this.neitherWin) if (this.margin == 0.0) return "0";
        else if (this._void) return "void";
        else return "?";
        else // there's a winner
        if (this.whiteWin) { // white wins
            if (resign) return "w+r";
            else if (time) return "w+t";
            else if (forfeit) return "w+f";
            else if (margin == Double.MAX_VALUE) return "w+";
            else return "w+" + margin;
        } else { // black wins
            if (resign) return "b+r";
            else if (time) return "b+t";
            else if (forfeit) return "b+f";
            else if (margin == Double.MAX_VALUE) return "b+";
            else return "b+" + margin;
        }
    }
    ...
    public boolean init(String s) {
        ...
        try {
            ...
            if (s.contains("w+r")) {
                ...
                if (s.length() == 2) this.margin = Double.MAX_VALUE;
                else {
            ...
            e.printStackTrace();
            if (DEBUG) System.err.println("Problem with \"" + original + "\"");
