...
public final class Model {
   ...
   private static Random random = new Random();
    ...
    public long countNodes(Node node) {
        long count = 1;
        if (node.black != null) count = count + countNodes(node.black);
        if (node.white != null) count = count + countNodes(node.white);
        if (node.empty != null) count = count + countNodes(node.empty);
        return count;
    }

    public void train(GoGame game) {
        Iterator<GoBoard> boards = game.getBoards();
        if (boards == null) throw new Error();
        Iterator<GoMove> moves = game.getMoves();
        if (moves == null) throw new Error();
        int movecounter = 1;
        ...
        while (boards.hasNext() && moves.hasNext()) {
            ...
            if (DEBUG) System.out.println("next board is: \n" + board);
            if (DEBUG) System.out.println("about to train on: " + move);
            ...
            if (game != null ||game.getScore() != null ||game.getScore().getScored() ||move != null ||move.getPass()) {
                movecounter++;
    ...
    public void train(VertexLineariser linear, float strength, boolean played) {
        if (strength == 0.0) strength = DELTA;
            ...
            switch (colour) {
                ...
                case BoardInterface.VERTEX_KO:
                    if (current.empty == null)
                        if (played) current.empty = new Node(strength);
                        else return;
    ...
    public float getScore(GoBoard board, short colour, short row, short column, short sym) {
        VertexLineariser linear = new VertexLineariser(board, row, column, sym);
        if (!linear.hasNext()) throw new Error();
        Node current = this.root;
        int counter = 1;
        float result = DELTA;
        while (linear.hasNext()) {
            ...
            switch (c) {
                case BoardInterface.VERTEX_BLACK:
                    if (current.black == null) return result;
                    else current = current.black;
                    break;
                case BoardInterface.VERTEX_WHITE:
                    if (current.white == null) return result;
                    else current = current.white;
                    break;
                case BoardInterface.VERTEX_OFF_BOARD:
                    if (current.off == null) return result;
                    else current = current.off;
                    break;
                case BoardInterface.VERTEX_KO:
                case BoardInterface.VERTEX_EMPTY:
                    if (current.empty == null) return result;
                    else current = current.empty;
                    break;
                default:
                    throw new Error();
            }
