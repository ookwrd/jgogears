...
public final  class Node  implements Comparable<Node>{
    ...
    public Node(short colour){
        if (colour == BoardInterface.VERTEX_OFF_BOARD) throw new Error ("Point off board");
        this.colour = colour;
    ...
    public int compareTo(Node node) {
        ...
        if (this.colour > node.colour)   return 1;
        if (this.colour < node.colour)   return -1;
        if (this.points.size() < node.points.size())   return -1;
        if (this.points.size() > node.points.size())   return 1;
    ...
    protected int calcLiberties(Graph graph){
        short libertyColour = this.getColour() == BoardInterface.VERTEX_BLACK ? BoardInterface.VERTEX_BLACK :  BoardInterface.VERTEX_WHITE;
        GoBoard board = graph.getBoard();
        if (points.size() == 0) throw new Error();
