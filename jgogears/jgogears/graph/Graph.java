...
public class Graph {
		...
		for (short i = 0; i < size; i++) {
			for (short j = 0; j < size; j++) {
				if (nodegrid[i][j] == null) {
					...
					if (DEBUG)
						System.err.println("created a node of size " + s);
				}
			}
		}
	}
	...
	public int spread(short i, short j, Node node) {
		...
		while (stack.size() > 0) {
			...
			if (i < 0 || j < 0 || j >= board.getSize() || i >= board.getSize())
				continue;
			if (nodegrid[i][j] != null) {
				if (nodegrid[i][j].getColour() == node.getColour()
						&& nodegrid[i][j] != node)
	...
	public Collection<Point> getNeighboringPoints(Point point) {
		...
		if (point.getColumn() > 0)
			result
					.add(this.getPointgrid()[point.getRow()][point.getColumn() - 1]);
		if (point.getColumn() < board.getSize() - 1)
			result
					.add(this.getPointgrid()[point.getRow()][point.getColumn() + 1]);
		if (point.getRow() > 0)
			result.add(this.getPointgrid()[point.getRow() - 1][point
					.getColumn()]);
		if (point.getRow() < board.getSize() - 1)
			result.add(this.getPointgrid()[point.getRow() + 1][point
					.getColumn()]);
		return result;
	}
